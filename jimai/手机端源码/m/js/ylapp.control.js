/*
 ...
 description:ylapp常用插件
 created:2014,08.18
 update:chenxue:xue.chen3g2win.com

 */
/*global ylapp,window */
ylapp.use("detect", function($, detect) {
    if (detect.os.ios) {
...
ylapp.extend(function(app, exports, module) {
    var $ = ylapp.require('dom');
    var appWin = ylapp.require('window');
    var locStorage = ylapp.require('locStorage');
    var view = ylapp.require('view');
    /*
     * 兼容pc，移动设备的封装
    ...
    function select(sel, cb) {
        ...
        $("select", $(sel)).on("change", function(evt) {
            ...
            ylapp.selectChange(evt.currentTarget);
            cb && cb(ele, ele.val());
        });
    }
    ...
    function touch(className, fun) {
        ...
        if (hasTouch) {
            ...
            $ele.on('tap', function() {
                if (ylapp.isFunction(ed.startCallFun)) {
    ...
    function objLength(obj) {
        ...
        if (ylapp.isArray(obj)) {
            return obj.length;
        } else {
            if (!ylapp.isPlainObject(obj)) {
                return c;
            }
            for (var i in obj) {
                c++;
