...
$headtitle=$LG['name.nav_45'];//积分获取记录
...
//echo $search;exit();
$query="select * from integral_czbak where {$where}  {$Mmy} {$order}";
    ...
    <div class="portlet-body" style="padding:10px;"> 
            ...
            <div class="form-group">
                <div class="col-md-0">
                  <div class="input-group input-xmedium date-picker input-daterange" data-date-format="yyyy-mm-dd">
                    <input type="text" class="form-control input-small" name="stime" value="<?=$stime?>" placeholder="<?=$LG['integral.integral_czbak_2'];//获分时间-起?>">
                    <span class="input-group-addon">-</span>
                    <input type="text" class="form-control input-small" name="etime" value="<?=$etime?>" placeholder="<?=$LG['integral.integral_czbak_4'];//获分时间-止?>">
                  </div>
                 </div>
              </div>
      ...
      <form action="save.php" method="post" name="XingAoForm">
        ...
        <table class="table table-striped table-bordered table-hover" >
          <thead>
            <tr>
              <th align="center"><a href="?<?=$search?>&orderby=type&orderlx=" class="<?=orac('type')?>"><?=$LG['integral.integral_czbak_5'];//加积分类型?></a></th>
               <th align="center"><a href="?<?=$search?>&orderby=fromtable&orderlx=" class="<?=orac('fromtable')?>"><?=$LG['integral.integral_bak_6'];//用途?></a> </th>
               <th align="center"><a href="?<?=$search?>&orderby=title&orderlx=" class="<?=orac('title')?>"><?=$LG['integral.integral_czbak_6'];//来源说明?></a> </th>
              <th align="center"><a href="?<?=$search?>&orderby=integral&orderlx=" class="<?=orac('integral')?>"><?=$LG['integral.integral_czbak_7'];//加积分?></a></th>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=addtime&orderlx=" class="<?=orac('addtime')?>"><?=$LG['integral.integral_czbak_10'];//加积分时间?></a></th>
...
?>
            ...
            <tr class="odd gradeX">
              ...
              <th align="center"><strong>
				<?php 
					$fe=FeData('integral_czbak',"count(*) as total,sum(`integral`) as integral","{$where} {$Mmy}");
