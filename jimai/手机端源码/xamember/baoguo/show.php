...
if(!$off_baoguo||!$member_per[$Mgroupid]['ON_Mbaoguo']){exit ("<script>alert('{$LG['baoguo.show_1']}');goBack();</script>");}
...
$rs=FeData('baoguo','*',"bgid='{$bgid}' {$Mmy}");

if(!$rs['bgid']){exit(XAts('checked'));}


$headtitle=cadd($rs['bgydh'])." {$LG['baoguo.show_2']}";
$alonepage=1;//单页形式
