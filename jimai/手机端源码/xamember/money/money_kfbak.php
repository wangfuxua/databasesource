...
if($so)
{
	...
	$search.="&so={$so}&key={$key}&stime={$stime}&etime={$etime}&fromtable={$fromtable}&tally={$tally}&fromCurrency={$fromCurrency}&toCurrency={$toCurrency}";
}
...
$query="select * from money_kfbak where {$where}  {$Mmy} {$order}";
    ...
    <div class="portlet-body" style="padding:10px;"> 
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
          <form class="navbar-form navbar-left"  method="get" action="?">
            ...
            <div class="form-group">
             ...
             <div class="form-group">
                <div class="col-md-0">
                     <select  class="form-control input-small select2me" name="tally" data-placeholder="<?=$LG['money.money_kfbak_1'];//月结?>" >
                     <option></option>
                     <?=Tally($tally,1)?>
                  	 </select>
               </div>
            ...
            <div class="form-group">
                <div class="col-md-0">
                  <div class="input-group input-xmedium date-picker input-daterange" data-date-format="yyyy-mm-dd">
                    <input type="text" class="form-control input-small" name="stime" value="<?=$stime?>" placeholder="<?=$LG['money.money_kfbak_2'];//扣款时间-起?>">
                    <span class="input-group-addon">-</span>
                    <input type="text" class="form-control input-small" name="etime" value="<?=$etime?>" placeholder="<?=$LG['money.money_kfbak_4'];//扣款时间-止?>">
                  </div>
                 </div>
              </div>
      ...
      <form action="save.php" method="post" name="XingAoForm">
        ...
        <table class="table table-striped table-bordered table-hover" >
          <thead>
            <tr>
              <th align="center"><a href="?<?=$search?>&orderby=type&orderlx=" class="<?=orac('type')?>"><?=$LG['money.money_kfbak_5'];//扣费类型?></a></th>
               <th align="center"><a href="?<?=$search?>&orderby=fromtable&orderlx=" class="<?=orac('fromtable')?>"><?=$LG['money.money_bak_8'];//用途?></a> </th>
               <th align="center"><a href="?<?=$search?>&orderby=title&orderlx=" class="<?=orac('title')?>"><?=$LG['money.money_bak_1'];//使用说明?></a> </th>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=addtime&orderlx=" class="<?=orac('addtime')?>"><?=$LG['money.money_kfbak_6'];//扣费时间?></a></th>
...
?>
           <tr class="odd gradeX">
              ...
              <td align="center"><?=money_kf($rs['type'])?>
...
?> 
     <tr class="odd gradeX">
      ...
      <td align="center">
        <strong>
        <?php 
        $query="select count(*) as total,sum(`toMoney`) as toMoney,toCurrency from money_kfbak where {$where} {$where_m} {$Mmy} group by toCurrency";
        ...
        <strong>
          <?php 
          $query="select count(*) as total,sum(`fromMoney`) as fromMoney,fromCurrency from money_kfbak where {$where} {$Mmy} {$where_m} group by fromCurrency";
          $sql=$xingao->query($query);
          while($rs=$sql->fetch_array())
          {
...
?>
   ...
   <?php }?>
            ...
            <tr class="odd gradeX">
              ...
              <strong>
<?php 
$query="select count(*) as total,sum(`toMoney`) as toMoney,toCurrency from money_kfbak where {$where} {$Mmy} group by toCurrency";
$sql=$xingao->query($query);
while($rs=$sql->fetch_array())
{
	if(spr($rs['toMoney'])){echo spr($rs['toMoney']).' '.cadd($rs['toCurrency']).'<br>';}
...
?>
              ...
              <strong>
<?php 
$query="select count(*) as total,sum(`fromMoney`) as fromMoney,fromCurrency from money_kfbak where {$where}   {$Mmy} group by fromCurrency";
$sql=$xingao->query($query);
while($rs=$sql->fetch_array())
{
	if(spr($rs['fromMoney'])){echo spr($rs['fromMoney']).' '.cadd($rs['fromCurrency']).'<br>';}
