/*! ylapp v1.1.0 web |  from 3g2win.com *//* Zepto v1.1.4 - zepto event ajax form ie - zeptojs.com/license */
var isWeb = true;
...
;(function($){
  ...
  $.ajax = function(options){
    ...
    if(settings.type && settings.contentType && settings.type.toUpperCase()=='POST'&&settings.contentType.toLowerCase() == 'application/json'){
        if(ylapp.isPlainObject(settings.data)){
...
;(function(global){
    ...
    //定义一个模块，或者插件
    ylapp.define = function(name,factory){
        if(isFunction(name)){
            name = '';
            factory = name;
        }
        ...
        var mod = {exports:{}};
        var res = factory.call(this,ylapp.require('dom'),mod.exports,mod);
        var exports = mod.exports || res;
        //模块已经存在
        if(name in ylapp){
            ylapp[name] = [ylapp.name];
            ylapp[name].push(exports);
        }else{
            ylapp[name] = exports;
        }
        return exports;
    ...
    */
    ylapp.extend = function(name,factory){
        if(arguments.length > 1 && isPlainObject(name)){
            return extend.apply(ylapp,arguments);
        }
        ...
        name = name ? name : this;
        var extendTo = ylapp.require(name);
    ...
    //代码入口
    ylapp.use = function(modules,factory){
        ...
        var args = [];
        args.push(ylapp.require('dom'));
        for(var i=0,len=modules.length; i<len; i++){
            args.push(ylapp.require(modules[i]));
        }
        return factory.apply(ylapp,args);
    };
    ...
    */
    ylapp.inherit = function(parent,protoProps, staticProps) {
...
;/*
    author:dushaobin
    email:shaobin.du@3g2win.com
    description:扩展zepto到ylapp dom 对象上
    扩展Backbone到ylapp Backbone 对象上
    扩展underscore到ylapp _ 对象上
    created:2014,08.18
    update:


*/
/*global ylapp,Zepto,Backbone,_,uexLog,window*/

//把zepto，导入到ylapp.dom 上
window.ylapp && window.ylapp.define('dom',function($,exports,module){
    module.exports = Zepto;
});

//把Backbone，导入到ylapp.Backbone 上
window.ylapp && ylapp.define('Backbone',function($,exports,module){
    module.exports = Backbone;
});

//把underscore，导入到ylapp._ 上
window.ylapp && ylapp.define('_',function($,exports,module){
    module.exports = _;
});

//把underscore，导入到ylapp.underscore 上
window.ylapp && ylapp.define('underscore',function($,exports,module){
    module.exports = _;
});

//扩展ylapp基础库能力
window.ylapp && ylapp.extend(function(ac,exports,module){

    /*
    打印日志到控制台，如果是ylapp应用打印到，响应的控制台
    @param * obj 任何类型
...
//扩展原声的dom对象
window.ylapp && ylapp.extend('dom',function(dom,exports,module){
    if(!ylapp.isylapp){
        return;
    }
...
;/*
    ...
    author:dushaobin
    email:shaobin.du@3g2win.com
    description:构建ylapp view模块
    create:2014.08.25
    update:______/___author___


*/
/* global ylapp,window,document*/
ylapp && ylapp.define('detect',function(ac,exports,module){
    var os = {};
    var browser = {};
...
;/*
 author:dushaobin
 email:shaobin.du@3g2win.com
 description:扩展encrypt 到ylapp对象上
 created:2014,08.21
 update:

 */
/*global ylapp*/
ylapp && ylapp.define('crypto', function($, exports, module) {
    /*
     扰乱s-box
...
*/
/*global ylapp,window*/
ylapp && ylapp.define('database',function($,exports,module){

    var eventEmitter = ylapp.require('eventEmitter');
    //var uexDataBaseMgr = window.uexDataBaseMgr || {};
    /*
    获取唯一操作符

    */
    var getOptId = ylapp.getOptionId;
    ...
    */
    ...
    var dbProto = {
        ...
        select:function(sql,callback){
            var that = this;
            var optId = getOptId();
            if(arguments.length === 1 && ylapp.isPlainObject(sql)){
                callback = sql.callback;
                sql = sql.sql;
            }
            if(ylapp.isFunction(callback)){
                if(!sql){
        ...
        transaction:function(sqlFun,callback){
            var that = this;
            var optId = getOptId();
            if(arguments.length === 1 && ylapp.isPlainObject(sqlFun)){
                callback = sqlFun.callback;
                sqlFun = sqlFun.sqlFun;
            }
            if(ylapp.isFunction(callback)){
                if(!ylapp.isFunction(sqlFun)){
    ...
    //实现eventEmitter能力
    ylapp.extend(dbProto,eventEmitter);
    ...
    var database = {
        ...
        */
        create:function(name,optId,callback){
            var argObj = null;
            if(arguments.length === 1 && ylapp.isPlainObject(name)){
                argObj = name;
                name = argObj.name;
                optId = argObj.optId;
    ...
    database = ylapp.extend(database,eventEmitter);
...
*/
/*global window,ylapp*/
window.ylapp && ylapp.define('device',function($,exports,module){
    ...
    */
    function getInfo(infoId,callback){
        if(arguments.length === 1 && ylapp.isPlainObject(infoId)){
    ...
    */
    function getDeviceInfo(callback){
        ...
        for(var i=0,len = 18;i<len;i++){
            getInfo(i,function(err,data){
                ...
                ylapp.extend(deviceInfo,singleInfo);
    ...
    /*ylapp.ready(function(){
        updateDeviceInfo(function(completeCount){
            ...
            ylapp.extend(deviceRes,deviceInfo);
        });
    });*/
...
;/*
    ...
    author:dushaobin
    email:shaobin.du@3g2win.com
    description:构建ylapp eventEmitter模块
    create:2014.08.18
    update:______/___author___
...
*/
/*global ylapp*/
ylapp && ylapp.define('eventEmitter',function($,exports,module){
    //事件对象
    var eventEmitter = {
        ...
        trigger:function(name,context){
            var args = [].slice.call(arguments,2);
            if(!this.__events || !ylapp.isString(name)){
                return;
            }
...
/*
    author:dushaobin
    email:shaobin.du@3g2win.com
    descript:构建ylapp file 模块
    created:2014.08.19
    update:____/____

*/
/*global ylapp,uexFileMgr*/

ylapp && ylapp.define('file',function($,exports,module){
    ...
    */
    var getOptionId = ylapp.getOptionId;
    ...
    */
    ...
    function exists(filePath,callback,optId){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(filePath)){
            argObj = filePath;
    ...
    function processReadGlobalQueue(err,data,dataType,optId){
        if(readGlobalQueue.length > 0){
            $.each(readGlobalQueue,function(i,v){
                if(v && v.cb && ylapp.isFunction(v.cb)){
    ...
    */
    function read(filePath,length,callback){
        ...
        if(ylapp.isFunction(length)){
            callback = length;
            length = -1;
        }
        callback = ylapp.isFunction(callback)?callback:function(){};
        length = length || -1;
    ...
    */
    function open(filePath,mode,callback){
        ...
        if(ylapp.isFunction(mode)){
            callback = mode;
            mode = 3;
        }
        mode = mode || 3;
        if(!ylapp.isString(filePath)){
    ...
    */
    function close(optId){
        if(arguments.length === 1 && ylapp.isPlainObject(optId)){
            optId = optId.optId;
        }
        if(!optId){
            return;
    ...
    */
    ...
    function deleteLocalFile(callback){
        if(ylapp.isPlainObject(callback)){
    ...
    */
    ...
    author:dushaobin
    email:shaobin.du@3g2win.com
    description:扩展zepto到ylappdom 对象上
    created:2014,08.20
    update:
...
*/
/*global ylapp,window*/
window.ylapp && ylapp.define('Model',function($,exports,module){
    var Backbone = ylapp.require('Backbone');
...
*/
/*global window,ylapp*/
ylapp && ylapp.define('request',function($,exports,module){
    var jsonpID = 0,
      document = window.document,
      key,
      name,
    ...
    var getXmlHttpId = ylapp.getOptionId;

    // trigger a custom event and return false if it was cancelled
    function triggerAndReturn(context, eventName, data) {
        ylapp.trigger(eventName,context,data);
    }

    // trigger an Ajax "global" event
    function triggerGlobal(settings, context, eventName, data) {
        if (settings.global) {
            return triggerAndReturn(context || ylapp, eventName, data);
        }
    }
  ...
  // serialize payload and append it to the URL for GET requests
  function serializeData(options) {
      if (options.processData && options.data && !ylapp.isString(options.data)){
          options.data = $.param(options.data, options.traditional);
      }
      if (options.data && (!options.type || options.type.toUpperCase() == 'GET')){
          options.data = $.param(options.data, options.traditional);
      ...
      var httpId = getXmlHttpId();
        ...
        if(settings.data && settings.contentType === false){
            for(name in settings.data){
                //fixed Number 类型bug
                if(ylapp.isPlainObject(settings.data[name])){
  ...
  // handle optional data/success arguments
  function parseArguments(url, data, success, dataType) {
    if (ylapp.isFunction(data)) {
        dataType = success;
        success = data;
        data = undefined;
    }
    if (!ylapp.isFunction(success)) {
        dataType = success;
        success = undefined;
    }
    return {
  ...
  function serialize(params, obj, traditional, scope){
    ...
    */
    ...
    function clearOffline(url,callback,data){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(url)){
            argObj = url;
            url = argObj['url'];
            data = argObj['data'];
            callback = argObj['callback'];
        ...
        var offlinedata = ylapp.locStorage.val(offlineKey);
        var offlineUrl;
        if(data){
            var paramsInfo = JSON.stringify(data);
            var fullUrl = url + paramsInfo;
            offlineUrl= ylapp.crypto.md5(fullUrl);
        }else{
            offlineUrl= ylapp.crypto.md5(url);
        }
                ...
                if(dataObj[offlineUrl]['data']){
                    ...
                    ylapp.file.remove({
                        ...
                        callback:function(err,data,dataType,optId){
                            ...
                            ylapp.locStorage.val(offlineKey,JSON.stringify(dataObj));
...
;/*
    author:dushaobin
    email:shaobin.du@3g2win.com
    description:扩展storage 到 ylapp 上
    created:2014,08.25
    update:


*/
/*global ylapp,window,unescape*/
ylapp && ylapp.define('locStorage',function($,exports,module){
    ...
    */
    function setValues(key){
        if(ylapp.isPlainObject(key)){
            for(var k in key){
                if(key.hasOwnPropery(k)){
                    setValue(k,key[k]);
                }
            }
        }else if(ylapp.isArray(key)){
...
*/
/*global ylapp*/
ylapp && ylapp.define('User',function($,exports,module){
    var Backbone = ylapp.require('Backbone');
    var db = ylapp.require('database');
...
*/
/*global window,ylapp*/
window.ylapp && ylapp.define('view',function($,exports,module){
    var _ = ylapp.require('underscore');
    var settings = {};
    /*
        配置模版参数
...
;/*
    ...
    author:dushaobin
    email:shaobin.du@3g2win.com
    description:构建ylapp window模块
    create:2014.08.18
    update:______/___author___


*/
/*global window,ylapp,uexWindow*/

window.ylapp && ylapp.define('window',function($,exports,module){
    ...
    */
    function monitorKey(id,enable,callback){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(id)){
            argObj = id;
            id = argObj['id'];
    ...
    */
    function open(name,data,aniId,type,dataType,width,height,animDuration,extraInfo){
		...
		try{
			extraInfo = ylapp.isString(extraInfo) ? extraInfo : JSON.stringify(extraInfo);
    ...
    */
    function close(animId,animDuration){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(animId)){
            argObj = animId;
    ...
    */
    function evaluatePopoverScript(name,popName,scriptContent){
        ...
        name = name || '';
        if(!ylapp.isString(popName) || !popName){
            return;           
        }
    ...
    */
    ...
    function setBounce(bounceType,startPullCall,downEndCall,upEndCall,color,imgSettings){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(bounceType)){
            argObj = bounceType;
    ...
    */
    function getBounceStatus(callback){
        if(arguments.length === 1 && ylapp.isPlainObject(callback)){
            callback = callback['callback'];
        }
        if(!ylapp.isFunction(callback)){
            return;
    ...
    */
    function setBounceType(type,color,flag,callback){
        if(arguments.length ===1 && ylapp.isPlainObject(type)){
            flag = type.flag;
            color = type.color;
                ...
                for(var i=0,len=bounceCallQueue.length;i<len;i++){
                    ...
                    if(currCallObj){
                        if(backType === currCallObj.type){
                            if(ylapp.isFunction(currCallObj.callback)){
    ...
    */
    function setBounceParams(position,data){
        if(arguments.length ===1 && ylapp.isPlainObject(position)){
            data = position.data;
            position = position.position;
        }
        if(ylapp.isPlainObject(data)){
    ...
    */
    ...
    function resetBounceView(position){
        if(ylapp.isPlainObject(position)){
            position = position['position'];
        }
    ...
    */
    ...
    function openToast(msg,duration,position,type){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(msg)){
            argObj = msg;
            msg = argObj['msg'];
    ...
    */
    ...
    function moveAnim(left,top,callback,duration){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(left)){
            argObj = left;
            left = argObj['left'] || 0;
            top = argObj['top'] || 0;
            callback = argObj['callback'];
    ...
    */
    ...
    function alphaAnim(alpha,callback,duration){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(alpha)){
            argObj = alpha;
    ...
    */
    ...
    function scaleAnim(x,y,z,callback,duration){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(x)){
            argObj = x;
            x = argObj['x'] || 1;
            y = argObj['y'] || 1;
            z = argObj['z'] || 1;
    ...
    */
    ...
    function rotateAnim(degrees,x,y,z,callback,duration){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(degrees)){
            argObj = degrees;
    ...
    */
    ...
    function setWindowFrame(dx,dy,duration,callback){
        if(arguments.length === 1 && ylapp.isPlainObject(dx)){
            argObj = dx;
            dx = argObj['dx'] || 0;
            dy = argObj['dy'] || 0;
            duration = argObj['duration'] || 250;
    ...
    */
    function windowConfirm(title,content,buttons,callback){
        if(arguments.length === 1 && ylapp.isPlainObject(title)){
        ...
        title = title || '提示';
        buttons = buttons || ['确定'];
        buttons = ylapp.isArray(buttons)?buttons:[buttons];
    ...
    */
    function subscribe(channelId,callback){
        if(arguments.length === 1 && ylapp.isPlainObject(channelId)){
        ...
        var funName = 'notify_callback_' + ylapp.getUID();
    ...
    */
    function openMultiPopover(popName,content,dataType, left, top, width, height,change, fontSize, flag, indexSelected,extraInfo){
        if(arguments.length === 1 && ylapp.isPlainObject(popName)){
    ...
    */
    ...
    function defaultStatusChange(state){
        ...
        if(ylapp.window.onResume){
            tmpResumeCall = ylapp.window.onResume;
        }
        if(ylapp.window.onPause){
            tmpPauseCall = ylapp.window.onPause;
        }
        
        if(state === 0){
            ylappWindow.emit('resume');
            tmpResumeCall && tmpResumeCall();
        }
        
        if(state === 1){
            ylappWindow.emit('pause');
    ...
    */
    ...
    /*
        ...
        兼容原始ylapp.frame.onSwipeLeft 和 ylapp.window.onSwipeLeft 方法
    ...
    */
    function defaultSwipeLeft(){
        ...
        if(ylapp.window.onSwipeLeft){
            tmpSwipeLeftCall = ylapp.window.onSwipeLeft;
        }
        
        if(ylapp.frame.onSwipeLeft){
            tmpFrameSLCall = ylapp.frame.onSwipeLeft;
        }
        
        ylappWindow.emit('swipeLeft');
        ylapp.frame && ylapp.frame.emit && ylapp.frame.emit('swipeLeft');
     ...
     /*
        ...
        兼容原始ylapp.frame.onSwipeRight 和 ylapp.window.onSwipeRight 方法
    ...
    */
    function defaultSwipeRight(){
        ...
        if(ylapp.window.onSwipeRight){
            tmpSwipeRightCall = ylapp.window.onSwipeRight;
        }
        
        if(ylapp.frame.onSwipeRight){
            tmpFrameSRCall = ylapp.frame.onSwipeRight;
        }
        
        ylappWindow.emit('swipeRight');
        ylapp.frame && ylapp.frame.emit && ylapp.frame.emit('swipeRight');
    ...
    */
    ...
    function setSlidingWindow(leftSliding,rightSliding,animationId,bg){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(leftSliding)){
            argObj = JSON.stringify(leftSliding);
        }else{
            argObj = {};
            if(ylapp.isPlainObject(leftSliding)){
                argObj.leftSliding = leftSliding;
            }else{
                argObj.leftSliding = JSON.parse(leftSliding);
            }
            if(ylapp.isPlainObject(rightSliding)){
    ...
    function toggleSlidingWindow(mark,reload){
        var argObj = null;
        if(!ylapp.isPlainObject(mark) && !JSON.parse(mark)){
            argObj = {};
            argObj.mark = mark;
    ...
    */
    function windowForward(animId,animDuration){
     ...
     /*
    在多窗口机制中，返回到上一个窗口:在多窗口机制中，用于返回上一个window，比如在A window中ylapp.window.open了B window，那么在Bwindow中返回Awindow就可使用此方法。
    @param Number animateId 动画类型Id
        animateId:动画类型Id
            0: 无动画
            1: 从左向右推入
    ...
    */
    ...
    /*
     调用ylapp.window.open方法且其中type为64时打开的主窗口ready方法中调用：预加载浮动窗口开始
     */
    ...
    /*
     调用ylapp.window.open方法且其中type为64时打开的主窗口ready方法中调用：预加载浮动窗口结束
     */
    ...
    */
    ...
    function insertPopoverAbovePopover (nameA,nameB){
        var argObj = null;
        if(arguments.length === 1 && ylapp.isPlainObject(nameA)){
            argObj = nameA;
            nameA = argObj["nameA"];
            nameB = argObj["nameB"];
        }
    ...
    */
    ...
    //导出接口
    var ylappWindow = module.exports = {
        open:open,
        close:close,
    ...
    ylapp.extend(ylappWindow,ylapp.eventEmitter);
...
*/
window.ylapp && ylapp.define('frame',function($,exports,module){
    var appWin = ylapp.require('window');
    
    var ylappFrame = module.exports = {
    ...
    ylapp.extend(ylappFrame,ylapp.eventEmitter);
...
*/
;(function() {
    var requestAjax = ylapp.request.ajax;
    //默认缓存文件路径
    var baseFilePath ='wgt://offlinedata/';
    //默认缓存到LocalStorage数据信息的key
    var offlineKey = 'offlinedata';

    var readFile = ylapp.file.read;
    var readSecureFile = ylapp.file.readSecure;
    var writeFile = ylapp.file.write;
    var writeSecureFile = ylapp.file.writeSecure;
    ...
    */
    function ajax(opts) {
        if (arguments.length === 1 && ylapp.isPlainObject(opts)) {
            var url;
            var expires;
            if(opts.data){
                var paramsInfo = JSON.stringify(opts.data);
                var fullUrl = opts.url + paramsInfo;
                url = ylapp.crypto.md5(fullUrl);
            }else{
                url = ylapp.crypto.md5(opts.url);
            }
            ...
            writeFileParams ={
                ...
                callback:function(err){
                    if(err == null){
                        ...
                        var offdata = ylapp.locStorage.val(offlineKey) || '{}';
                        var offdataObj = JSON.parse(offdata);
                        offdataObj[filename] = saveData;
                        ylapp.locStorage.val(offlineKey, JSON.stringify(offdataObj)); 
                    }  
                }
            }
            if(opts.crypto){
    ...
    */
    ...
    ylapp.extend(ylapp.request, {
        ajax : ajax
    });
})();

ylapp.define('ajax', function($, exports, module){
    module.exports = ylapp.request.ajax;
});
;;
/*
    author:jiaobingqian
    email:bingqian.jiao@zymobi.com
    descript:构建ylapp download 模块
    created:2016.03.02
    update:____/____

*/
/*global ylapp,uexDownloaderMgr*/

ylapp && ylapp.define('download',function($,exports,module){
    ...
    */
    ...
    function processDownloadQueue(optId,fileSize,percent,status){
        ...
        if(status == 0){//DownLoading
            downloadCall && ylapp.isFunction(downloadCall) &&  downloadCall(optId,fileSize,percent,status);
        }else if(status == 1) {// finish download 
            successCall && ylapp.isFunction(successCall)  && successCall(optId,fileSize,percent,status);
        }else if(status == 2) {// download error
            errorCall && ylapp.isFunction(errorCall)  && errorCall(optId,fileSize,percent,status);
        }else if(status == 3){// cancel download
            cancelCall && ylapp.isFunction(cancelCall)  && cancelCall(optId,fileSize,percent,status);
        }
    ...
    function setHeaders(optId,info){
        ...
        if(!ylapp.isPlainObject(info)) throw new Error('info is not json object!');
 ...
 */
ylapp.define("icache", function($, exports, module) {
    var opid = 1000;
    var CACHE_PATH = "box://icache/";
    function iCache(option) {
        var self = this;
        ylapp.extend(this, ylapp.eventEmitter);
        self.waiting = [];
        self.running = [];
        self.option = $.extend({
            maxtask : 3
    ...
    iCache.prototype = {
        ...
        _success : function(fpath, session) {
            ...
            ylapp.download.close(session.dlId);
            if (session.success) {
        ...
        _download : function(session) {
            var self = this;
            var fid = ylapp.crypto.md5(session.url);
            ...
            self.once("FS" + session.id, function(data) {
                if (data) {
                    ...
                    session.dlId = ylapp.download.create(null, function(err, data, dataType, optId) {
                        if (!err) {
                            session.header && ylapp.download.setHeaders(session.dlId, session.header);
                            ylapp.download.run(session.dlId, session.url, fpath, '0', downloadCB,downloadCB,downloadCB,downloadCB)
                        } else
            ...
            ylapp.file.exists(fpath,function(err,data,dataType,optId){
        ...
        run : function(option) {
            ...
            ylapp.file.getRealPath("box://",function(err,data,dataType,optId){
                self.realpath = data;
            self.emit("NEXT_SESSION");
            });
        },
...
;/*
    author:jiaobingqian
    email:bingqian.jiao@zymobi.com
    description:构建ylapp widget模块
    create:2015.11.26
    update:______/___author___

*/
/*global window,ylapp,uexWidget*/
window.ylapp && ylapp.define('widget',function($,exports,module){
    ...
    function startWidget(appId,animId,funName,info,animDuration,callback) {
        var uexObj = null;
        if (arguments.length ===1 && ylapp.isPlainObject(appId)) {
            uexObj = appId;
            appId = uexObj['appId'];
    ...
    function finishWidget(resultInfo,appId,isWgtBG) {
        var uexObj = null;
        if (arguments.length ===1 && ylapp.isPlainObject(resultInfo)) {
            uexObj = resultInfo;
    ...
    function loadApp(appInfo) {
        var uexObj = null;
        if (arguments.length ===1 && ylapp.isPlainObject(appInfo)) {
            uexObj = appInfo;
    ...
    function startApp(startMode,mainInfo,addInfo,optInfo,callback) {
        var uexObj = null;
        if (arguments.length ===1 && ylapp.isPlainObject(startMode)) {
            uexObj = startMode;
    ...
    function installApp(appPath) {
        var uexObj = null;
        if (arguments.length ===1 && ylapp.isPlainObject(appPath)) {
            uexObj = appPath;
    ...
    function setPushNotifyCallback(cbFunction) {
        if (arguments.length === 1 && ylapp.isPlainObject(cbFunction)) {
    ...
    function setPushInfo(uId,uNickName) {
        var uexObj = null;
        if (arguments.length ===1 && ylapp.isPlainObject(uId)) {
            uexObj = uId;
            uId = uexObj['uId'];
     ...
     */
     function setPushState(state) {
        var uexObj = null;
        if (arguments.length === 1 && ylapp.isPlainObject(state)) {
            uexObj = state;
            state = uexObj['state'];
        }
     ...
     function isAppInstalled(appData,callback) {
        var uexObj = {};
        var res = null;
        if(arguments.length === 1 && ylapp.isPlainObject(appData)){
            uexObj = appData;
    ...
    */
    ...
    function defaultLoadByOtherApp(){
        ...
        if(ylapp.widget.onLoadByOtherApp){
            tmpLoadByOtherAppCall = ylapp.widget.onLoadByOtherApp;
        }
        ylappWidget.emit('loadByOtherApp');
    ...
    */
    ...
    function defaultSuspend(){
        ...
        if(ylapp.widget.onSuspend){
            tmpSuspendCall = ylapp.widget.onSuspend;
        }
        ylappWidget.emit('suspend');
    ...
    */
    ...
    function defaultResume(){
        var tmpResumeCall = null;
        if(ylapp.widget.onResume){
            tmpResumeCall = ylapp.widget.onResume;
        }
        ylappWidget.emit('resume');
        tmpResumeCall && tmpResumeCall();
    ...
    //导出接口
    var ylappWidget = module.exports = {
    ...
    ylapp.extend(ylappWidget,ylapp.eventEmitter);
...
*/
/*global window,ylapp,uexWidgetOne*/
ylapp.define('widgetOne', function($, exports, module) {
    /*
    获取平台信息,回调中返回获取结果
    ...
    function getPlatform(callback) {
        ...
        if (!ylapp.isFunction(callback)) return;
    ...
    */
    function exit(flag) {
        if (arguments.length === 1 && ylapp.isPlainObject(flag)) {
            flag = flag['flag'];
        }
        //flag是number类型
    ...
    var ylappWidgetOne = module.exports = {
    ...
    //ylapp.extend(ylappWidgetOne, ylapp.eventEmitter);
});;
;/*!
...
;(function(factory){
    ...
    ylapp.ready(function(){
        ...
        {
            window.lazyICache = ylapp.icache({maxtask:1});
        }
    })
    ...
    _.extend(Service.prototype, Backbone.Events, {
        ...
        getLockKey : function(string) {
            if (string) {
                ...
                var md5 = ylapp.crypto.md5(string);
                return md5;
            }
        },
        /**
