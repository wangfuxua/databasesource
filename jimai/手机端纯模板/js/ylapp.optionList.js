ylapp.define("optionList", function($, exports, module) {
...
<%}%>\
    ...
    var hiddenLineTmp = ylapp.view.template(model_hiddenLine);
    var viewid = 1;
    function isWindows() {
    ...
    */
    function OptionListView(option) {
        ...
        if(uexWindow.getBounce && typeof uexWindow.getBounce === 'function'){
            ylapp.window.getBounceStatus(function(dt,dataType,opId){
                bounceState = dt;
            });
        }
        ...
        */
        buildListview : function(data,group) {
            ...
            $(document).on("touchstart", ".scroll-bar", function(evt) {
                if(bounceState == 1){
                    ylapp.window.disableBounce();
                }
                //ylapp.window.setMultilPopoverFlippingEnbaled(1);
            ...
            $(document).on("touchend", ".scroll-bar", function(evt) {
                ...
                //ylapp.window.setMultilPopoverFlippingEnbaled(0);
                if (!scrollEnable)
                    return;
