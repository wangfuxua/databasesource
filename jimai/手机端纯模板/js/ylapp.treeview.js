ylapp.define("treeview", function($, exports, module) {
    ...
    var listRender = ylapp.view.template(listTmp);
    ...
    Treeview.prototype = {
        ...
        getContent : function(data, settings) {
            ...
            var index = 0;
            if (!settings.type) {
                if ((ylapp.isArray(contentData) || ylapp.isPlainObject(contentData)) && !$.zepto.isZ(contentData)) {
                    var warper = $('<div class="listview"></div>');
                    settings.option.selector = warper;
                    var lv = ylapp.listview(settings.option);
                    lv.set(contentData);
                ...
                var plugin = ylapp[settings.type];
                if (plugin) {
